...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if not detection_made:
                        if vid_path != abs_path+save_path:  # new video
                            vid_path = abs_path+save_path
                        ...
                        if os.path.exists(vid_path) and os.access(vid_path, os.W_OK):
                            vid_writer.write(im0)
                            print(vid_path)
                        else:
                            print(f"Error: {vid_path} does not exist or is not writable.")
                        print(save_path)
                    else:
                        save_path_no_ext = save_path.replace(".mp4", "")
                        cv2.imwrite(abs_path+save_path_no_ext+"_"+str(count)+".jpg", im0) 
            ...
            if detection_made and count == 10:
                break
